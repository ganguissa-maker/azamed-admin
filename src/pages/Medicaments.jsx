// src/pages/Medicaments.jsx — Admin avec modification
...
export default function Medicaments() {
  ...
  const { mutate: add, isPending: adding } = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries(['admin-meds']); setForm(VIDE); setShowForm(false); toast.success('Médicament ajouté !'); },
    onError: (e) => toast.error(e.response?.data?.error || 'Erreur'),
  });

  const { mutate: edit, isPending: saving } = useMutation({
    mutationFn: () => api.put(`/admin/medicaments/${editing.id}`, editForm),
    onSuccess: () => { queryClient.invalidateQueries(['admin-meds']); setEditing(null); toast.success('Médicament modifié !'); },
    onError: (e) => toast.error(e.response?.data?.error || 'Erreur'),
  });

  const { mutate: remove } = useMutation({
    mutationFn: (id) => api.delete(`/admin/medicaments/${id}`),
    onSuccess: () => { queryClient.invalidateQueries(['admin-meds']); setToDelete(null); toast.success('Médicament supprimé.'); },
  ...
  const startEdit = (med) => {
    ...
    setEditForm({
      ...
      dci:                 med.dci || '',
      classeTherapeutique: med.classeTherapeutique || '',
      forme:               med.forme || '',
      dosage:              med.dosage || '',
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          <p className="text-gray-500 text-sm">{data?.pagination?.total ?? 0} médicaments</p>
        </div>
        ...
        <div className="card border-2 border-primary-100 mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-4">
            {CHAMPS.map(({ f, l, required }) => (
              <Field key={f} label={l} name={f} required={required} value={form[f]} onChange={handleChange(f)}/>
            ))}
          </div>
          <div className="flex gap-2">
            <button onClick={() => add()} disabled={adding || !form.nomCommercial || !form.dci} className="btn-primary disabled:opacity-50">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                {['Nom commercial','DCI','Classe','Forme','Dosage','Actions'].map((h) => (
            ...
            <tbody className="divide-y divide-gray-50">
              {isLoading ? (
                ...
                editing?.id === med.id ? (
                  // ─ Mode édition ─
                  <tr key={med.id} className="bg-primary-50">
                    {['nomCommercial','dci','classeTherapeutique','forme','dosage'].map((f) => (
                      <td key={f} className="px-3 py-2">
                        <input value={editForm[f]} onChange={handleEditChange(f)}
                          className="w-full px-2 py-1.5 border border-primary-200 rounded-lg text-sm focus:outline-none bg-white"/>
                      </td>
                    ))}
                    <td className="px-3 py-2">
                      <div className="flex gap-1">
                        <button onClick={() => edit()} disabled={saving}
                          className="p-1.5 bg-green-100 text-green-600 hover:bg-green-200 rounded-lg transition-colors" title="Enregistrer">
                          <Check size={14}/>
                        </button>
                        <button onClick={() => setEditing(null)}
                          className="p-1.5 bg-gray-100 text-gray-500 hover:bg-gray-200 rounded-lg transition-colors" title="Annuler">
                          <X size={14}/>
                        </button>
                      </div>
                    </td>
                  ...
                  <tr key={med.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-xs text-gray-500 max-w-[160px] truncate">{med.classeTherapeutique}</td>
                    <td className="px-4 py-3 text-sm text-gray-500">{med.forme}</td>
                    <td className="px-4 py-3 text-sm text-gray-500">{med.dosage}</td>
                    <td className="px-4 py-3">
                      <div className="flex gap-1">
                        <button onClick={() => startEdit(med)} title="Modifier"
                          className="p-1.5 text-blue-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
                          <Pencil size={13}/>
                        </button>
                        <button onClick={() => setToDelete(med)} title="Supprimer"
